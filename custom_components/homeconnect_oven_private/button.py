    ...
    for oven in ovens:
        entities.extend(
            [
                ...
                RefreshSnapshotButton(manager, oven.private_ha_id),
            ]
        )
...
class RefreshSnapshotButton(HomeConnectPrivateOvenEntity, ButtonEntity):
    """Force the private oven camera entity to fetch the latest snapshot."""

    def __init__(self, manager: HomeConnectPrivateOvenManager, private_ha_id: str) -> None:
        super().__init__(manager, private_ha_id, "refresh_snapshot")
    ...
    async def async_press(self) -> None:
        ...
        camera_entity_id = registry.async_get_entity_id(
            "camera",
            DOMAIN,
            f"{self._private_ha_id}_camera",
        )
        if not camera_entity_id:
            raise HomeAssistantError("Private oven camera entity was not found.")
